...
int setStudent(student* _dst) {
    ...
    _dst->name = (char*)calloc(nameLen, sizeof(char));  // 이름을 저장할 메모리 할당
    heapUsage += sizeof(char) * nameLen;                // 할당된 메모리 크기 누계
...
int main() {
    ...
    // numStudent 개수 만큼의 student 사이즈 메모리 할당
    student* students = (student*)malloc(sizeof(student) * numStudent);
    memset(students, 0, sizeof(student) * numStudent);  // 0 으로 초기화
    heapUsage += sizeof(student) * numStudent;          // 할당된 메모리 크기 누계

    // numStudent 개수 만큼 랜덤한 학생정보 저장
